...
def normalize_ohlcv(data: pd.DataFrame, ticker: str = "") -> pd.DataFrame:
    """Limpieza segura para evitar mezclar datos entre tickers.

    yfinance a veces devuelve MultiIndex o columnas duplicadas. Esta función
    conserva solo OHLCV del ticker correcto y elimina duplicados.
    """
    ...
    if isinstance(out.columns, pd.MultiIndex):
        # Caso columnas tipo (Price, Ticker) o (Ticker, Price).
        levels0 = [str(x).upper() for x in out.columns.get_level_values(0)]
        levels1 = [str(x).upper() for x in out.columns.get_level_values(1)]
        t = str(ticker).upper().replace("-", ".")
        if t in levels0:
            out = out.xs(t, axis=1, level=0)
        elif t in levels1:
            out = out.xs(t, axis=1, level=1)
        else:
            # Descarga individual: normalmente basta con el primer nivel.
    ...
    for col in out.columns:
        ...
        elif c in ["volume", "vol"]: rename[col] = "Volume"
    ...
    for c in needed + (["Volume"] if "Volume" in out.columns else []):
...
def download_data(ticker: str, period: str = DEFAULT_PERIOD, interval: str = "1d") -> pd.DataFrame:
    """Descarga segura ticker por ticker.

    Usamos Ticker.history como método principal para evitar el bug de mezcla
    donde META podía recibir precio de COST/NVDA. yf.download queda solo
    como fallback y siempre con limpieza robusta.
    """
    ...
    for attempt in range(3):
        try:
            ...
            data = normalize_ohlcv(data, ticker)
...
def analyze_ticker(ticker: str):
    ...
    if pd.isna(price) or price <= 0 or pd.isna(prev_price) or prev_price <= 0:
        return None
...
def alert_line(row: dict) -> str:
    ...
    return (
        ...
        f"Precio: ${fmt(row.get('Precio'), 2)} | RSI: {fmt(row.get('RSI'), 1)} | Data: {row.get('Data Status', 'OK')}"
    )
